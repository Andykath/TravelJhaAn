   ...
   $cedula=$_SESSION['cedula'];
	...
	//echo "pre_fecha".$fecha; echo  "pre_habitacion".$habitacion."pre_servicio".$servicio."pre_total".$total."pre_cant_per".$cantper."fk_via_id_origen".$banco."fk_via_id_destino".$banco1;
			...
			mysql_query("UPDATE `presupuesto` SET  `pre_fecha`= '$fecha', `pre_habitacion`=NULL, `pre_servicio`='$servicio', `pre_total`= '$total',`pre_cant_per`='$cantper', `fk_via_id_origen`='$banco', `fk_via_id_destino`='$banco1',  `pre_abono`=0, `pre_status`='No comprado' where pre_id='$id'") or die (mysql_error());
				...
				$hola='a_presupuesto.php?mensaje=2';
	      ...
	      else
		  {
			...
			$panelcuentas = new Panel("../html/u_crear_presupuesto_aereo.html");
			$panelcuentas->add("form",'<form name="form1" method="post" action="../php/a_editar_presupuesto_aereo.php?">');
			 $panelcuentas->add("Aqui",'<a href="../php/a_presupuesto.php">Volver<<</a>');
			$panelcuentas->add("fecha",$descripcion);
			$panelcuentas->add("id",$id);
			//$panelcuentas->add("servicio_actual",$servicio);
			//$panelcuentas->add("paseo_actual",$paseo);
			//$panelcuentas->add("hotel_actual",$hotel);
			//$origen1=$origen."/".$ciudado;
			//$panelcuentas->add("origen_actual",$via);
			//$panelcuentas->add("id",'<tr><td>Id:</td><td>'.$id.'</td></tr>');
			 if ($mensaje==1)
			 {
	        $select='';
			...
			// echo('entra');
			//$panelcuentas->add("form",'<form name="form1" method="post" action="../php/a_editar_presupuesto_aereo.php?selected='.$via.'&fechai='.$descripcion.'&selected2=k&selected3=a>"');
			//echo '../php/a_editar_presupuesto_aereo.php?selected='.$via.'&fechai='.$descripcion.'&selected2=k&selected3=a>';
			...
			if ($selected && $fechai && ($selected2=="k") && ($selected3=="a") && $id)//aqui no va a entrar porq el 2 no es k es ke no tiene ke entrar ahi sino en el otro 
