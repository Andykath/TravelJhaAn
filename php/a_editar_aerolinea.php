<?php
	...
	extract($_POST);
		  ...
		  {// aqui validar que los datos esten bien y hacer update
               ...
               if (($estadio_viejo== $cue_numero)&&($aforo_viejo==$tipo)&&($ciudad_vieja==$banco) && ($fecha_vieja == $fecha))    
				...
				else 
				{
					...
					mysql_query("UPDATE `aerolinea` SET  `aer_nombre` =  '$cue_numero',  `aer_fecha_inicio` =  '$fecha',`fk_des_id` =  '$banco' WHERE  `aerolinea`.`aer_id` = $cue_id");
					...
					{//1
					   ...
					   $result1= mysql_query("SELECT * FROM aerolinea a WHERE a.aer_id ='$cue_numero' AND a.fk_des_id = '$banco'");
		  ...
		  {//aqui pinta el editor por primera vez	
					...
					$panelestadios = new Panel("../html/a_crear_aerolinea.html");
					$panelestadios->add("form",'<form name="form1" method="post" action="../php/a_editar_aerolinea.php?cue_id='.$id.'">');
