<?php
	...
	extract($_POST);
		  ...
		  {// aqui validar que los datos esten bien y hacer update
					...
					// ech/o($actual_1);
					 // echo($hot_id);
					  ...
					  mysql_query("UPDATE `hot_ser` SET  `fk_ser_id` =  '$banco', `fk_hot_id` =  '$hot_id' WHERE  `hot_ser`.`hot_id` = $actual_1");
					
					$hola='a_servicios_hoteles.php?cue_id='.$hot_id.'';
		  ...
		  {//aqui pinta el editor por primera vez	
					...
					$panelestadios = new Panel("../html/a_crear_servicio_hotel.html");
					$panelestadios->add("form",'<form name="form1" method="post" action="../php/a_modificar_servicio_hotel.php?hot_id='.$hotel.'&actual_1='.$actual.'">');
