<?php
	...
	$panelcuentas = new Panel("../html/u_viaje_undestino_conestadia_maritimo.html");
	
	
	

	$result= mysql_query("SELECT v.via_id, v.via_tipoviaje, v.via_fecha_ini, v.via_fecha_fin, v.via_hora_ini, v.via_hora_fin, v.via_cant_per, a.cru_nombre, d.des_nombre, o.des_nombre, ho.flo_nombre, ha.hab_id, e.est_fecha_ini, e.est_fecha_fin
FROM via i, via f, destino d, destino o, crucero a, flota ho, viaje v, habitacion ha, presupuesto p, estadia e
WHERE v.fk_via_id_origen = i.via_id
AND v.fk_via_id_destino = f.via_id
AND i.fk_des_id = d.des_id
AND f.fk_des_id = o.des_id
AND i.fk_cru_id = a.cru_id
AND v.fk_pre_id = p.pre_id
AND p.fk_per_cedula='$cedula'
AND e.fk_via_id = v.via_id
AND e.fk_hab_id = ha.hab_id
AND ha.fk_cru_id = ho.flo_id
