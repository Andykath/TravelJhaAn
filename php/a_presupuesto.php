<?php
	...
	$panelcuentas = new Panel("../html/a_general_presupuesto.html");
	...
	$presu= mysql_query("SELECT p. *  FROM presupuesto p WHERE p.pre_id IN (SELECT p.pre_id FROM servicio s, via v, aerolinea e WHERE p.fk_via_id_origen = v.via_id AND p.pre_servicio = s.ser_id AND v.fk_aer_id = e.aer_id ORDER BY pre_id) OR p.pre_id IN (SELECT p.pre_id
FROM via v, aerolinea e WHERE p.fk_via_id_origen = v.via_id AND p.pre_servicio IS NULL AND v.fk_aer_id = e.aer_id ORDER BY pre_id)");
    ...
    <tr align="center">
	  ...
	  <td width="200"><a href="../php/a_editar_presupuesto_aereo.php?descripcion='.$row['pre_fecha'].'&id='.$row['pre_id'].'&servicio='.$ser_id.'&hotel='.$row2['hot_id'].'&habitacion='.$row2["hab_id"].'&origen='.$row3['aer_nombre'].'&via='.$row['fk_via_id_origen'].'&destino='.$row['fk_via_id_destino'].'&total='.$row['pre_total'].'&mensaje=1'.'">Modificar</a> <a href="../php/a_eliminar_presupuesto_aereo.php?cue_id='.$row["pre_id"].'" onclick="return confirmar()">Eliminar</a></td>
    </tr>';
