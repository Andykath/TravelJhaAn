<?php
	...
	$panelcuentas = new Panel("../html/a_general_presupuesto_crucero.html");
	...
	$presu= mysql_query("SELECT p. *  FROM presupuesto p WHERE p.pre_id IN (SELECT p.pre_id FROM servicio s, via v, crucero e WHERE p.fk_via_id_origen = v.via_id AND p.pre_servicio = s.ser_id AND v.fk_cru_id = e.cru_id ORDER BY pre_id) OR p.pre_id IN (SELECT p.pre_id
FROM via v, crucero e WHERE p.fk_via_id_origen = v.via_id AND p.pre_servicio IS NULL AND v.fk_cru_id = e.cru_id ORDER BY pre_id)");
	...
	else
	{
		...
		//echo $hab;
		$hotel= mysql_query("SELECT h.cru_nombre, h.cru_id, a.hab_id, a.hab_costo FROM crucero h, habitacion a, presupuesto p where a.fk_cru_id=h.cru_id and  a.hab_id='$hab' and p.pre_id='$id'");
		$row2 = mysql_fetch_array($hotel);
		$hot=$row2["cru_nombre"].', #Habitacion: '.$row2["cru_id"];
	}
	...
	$destino= mysql_query("SELECT d.des_nombre as pais, o.des_nombre as ciudad, a.aer_nombre FROM aerolinea a, destino d, destino o, via v, presupuesto p where p.fk_via_id_destino=v.via_id and v.fk_des_id=o.des_id and o.fk_des_id=d.des_id and   p.pre_id='$id'");
    ...
    <tr align="center">
	  ...
	  <td width="200"><a href="../php/a_editar_presupuesto_crucero.php?descripcion='.$row['pre_fecha'].'&id='.$row['pre_id'].'&servicio='.$ser_id.'&hotel='.$row2['cru_id'].'&habitacion='.$row2["hab_id"].'&origen='.$row3['cru_nombre'].'&via='.$row['fk_via_id_origen'].'&destino='.$row['fk_via_id_destino'].'&total='.$row['pre_total'].'&mensaje=1'.'">Modificar</a> <a href="../php/a_eliminar_presupuesto_crucero.php?cue_id='.$row["pre_id"].'" onclick="return confirmar()">Eliminar</a></td>
    </tr>';
