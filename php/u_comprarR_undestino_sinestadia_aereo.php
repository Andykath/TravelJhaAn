  ...
  $millas=mysql_query("SELECT via_millas FROM  via v where  via_id='$viadestino'");
		    ...
		    $pago2=$roc['max'];
					 ...
					 $antes=mysql_query("SELECT `pre_abono` FROM  presupuesto WHERE pre_id='$id'");
						 ...
						 $hola1='u_reserva_undestino_sinestadia_aereo1.php?mensaje=2';
				           header("Location:$hola1");
		...
		{
			$hola1='u_flota_errorR_sinestadia_aereo.php?mensaje=2';
						header("Location:$hola1");
			}
		  ...
		  {//aqui pinta el editor por primera vez	
					...
					$admin= new Panel("../html/usuario.html");
					$admin->add("body",'<body onLoad = "actual(3)" >');
					$panelestadios = new Panel("../html/u_reserva_comprar_undestino_sinestadia_aereo.html");
					$panelestadios->add("form",'<form name="form1" method="post" action="../php/u_comprarR_undestino_sinestadia_aereo.php?id='.$id.'&aerolinea='.$aerolinea.'&origen='.$origen.'&destino='.$destino.'&total='.$total.'&cantper='.$cantper.'&tipo='.$tipo.'&fechapago='.$fechapago.'&numero='.$numero.'&combo='.$combo.'&nombre='.$nombre.'&cvv2='.$cvv2.'&monto='.$monto.'&monto1='.$monto1.'&monto2='.$monto2.'&fecha='.$fechai.'&fecha1='.$fechaf.'">');
