<?php
	...
	$result= mysql_query("SELECT v.via_id,v.via_fecha_ini,p.pre_abono,p.pre_habitacion,p.pre_cant_per,p.pre_paseo,p.pre_total,p.fk_via_id_origen,p.fk_via_id_destino,ho.hot_nombre,ho.hot_id,ha.hab_id,s.ser_nombre,p.pre_id FROM  presupuesto p, hotel ho, servicio s, habitacion ha, viaje v WHERE  p.fk_per_cedula='$cedula' AND p.pre_status='No comprado' and p.pre_paseo IS NOT NULL and p.pre_habitacion IS NOT NULL AND p.pre_habitacion=ha.hab_id AND ha.fk_hot_id=ho.hot_id AND p.pre_servicio=s.ser_id and v.fk_pre_id=p.pre_id and v.via_tipo='Reserva' ORDER BY v.via_id");
