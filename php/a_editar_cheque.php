	...
	extract($_POST);
	      ...
	      if($numero_viejo)
		  {
			  //echo $numero."numero_viejo".$numero_viejo."-".$cvv2."cvv2_viejo".$cvv2_viejo."-".$banco."banco_viejo".$banco_viejo."-".$cliente."cliente_viejo".$cliente_viejo."-".$nombrede."nombre_viejo".$nombrede_viejo."-".$fecha."fecha_viejo".$fecha_viejo;
				if (($numero_viejo== $numero)&&($cuenta_viejo==$cuenta)&&($nombre_viejo==$nombre)&&($fecha_viejo==$fecha)&&($banco_viejo==$banco)&&($cliente_viejo==$cliente))    
				{	
				...
				else 
				{
				    ...
				    if (($numero_viejo== $numero)&&($cuenta_viejo==$cuenta)&&($nombre_viejo==$nombre)&&($fecha_viejo==$fecha)&&($banco_viejo==$banco))
					{
					
					mysql_query("UPDATE `cheque` SET  `che_num` =  '$numero', fk_ban_id='$banco', che_fechaven='$fecha', fk_per_cedula='$cliente', `che_cuenta` =  '$cuenta', `che_nombre` =  '$nombre' WHERE  che_id = $cue_id") or die ('Error en el numero'.mysql_error());;
					
					$hola='a_cheque.php?mensaje=2';
				    header("Location:$hola");	
					...
					else 
					{
					   ...
					   if (($cuenta_viejo==$cuenta)&&($nombre_viejo==$nombre)&&($fecha_viejo==$fecha)&&($banco_viejo==$banco)&&($cliente_viejo==$cliente))
					  {
				    ...
				    header("Location:$hola");	
						...
						{
							if (($numero_viejo== $numero)&&($cuenta_viejo==$cuenta)&&($nombre_viejo==$nombre)&&($fecha_viejo==$fecha)&&($cliente_viejo==$cliente))
							{
								//echo "cambio"; 
								mysql_query("UPDATE `cheque` SET  `che_num` =  '$numero', fk_ban_id='$banco', che_fechaven='$fecha', fk_per_cedula='$cliente', `che_cuenta` =  '$cuenta', `che_nombre` =  '$nombre' WHERE  che_id = $cue_id") or die ('Error en el numero'.mysql_error());;
		  ...
		  {//aqui pinta el editor por primera vez	
					...
					$paneleditar = new Panel("../html/a_crear_cheque.html");
					$paneleditar->add("form",'<form name="form1" method="post" action="../php/a_editar_cheque.php?cue_id='.$id.'">');
					$paneleditar->add("numero",$numero);
					$paneleditar->add("id_viejo",$id);
					$paneleditar->add("cuenta",$cuenta);
					$paneleditar->add("nombre",$nombre);
