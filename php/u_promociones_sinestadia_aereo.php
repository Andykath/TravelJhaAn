<?php
	...
	$panelcuentas = new Panel("../html/u_promocion_sinestadia_aereo.html");
...
FROM promocion p, via v, via f, destino d, destino o ,aerolinea a WHERE p.fk_via_id= v.via_id AND v.fk_des_id = d.des_id  and  p.fk_via_id2= f.via_id AND f.fk_des_id = o.des_id AND v.fk_aer_id=a.aer_id and v.fk_cru_id IS NULL AND v.fk_ter_id IS NULL AND p.fk_hab_id IS NULL ORDER BY p.pro_id");
    ...
    <tr align="center">
	  ...
	  <td width="200"><a href="../php/u_comprar_promocion_sinestadia_aereo.php?nombre='.$row['pro_nombre'].'&id='.$row['pro_id'].'&desc='.$row['pro_descuento'].'&dur='.$row['pro_durac_viaje'].'&aerolinea='.$row['aer_nombre'].'&aerid='.$row['aer_id'].'&fkvia='.$row[6].'&fkvia2='.$row[7].'&hotel='.$row['hot_nombre'].'&origen='.$row[9].'&destino='.$row[10].'&cantper='.$row['via_cant_per'].'&viacosto='.$row['via_costo'].'">Comprar</a>
    </tr>';
