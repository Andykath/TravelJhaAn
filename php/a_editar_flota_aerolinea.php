<?php
	...
	extract($_POST);
		  ...
		  {// aqui validar que los datos esten bien y hacer update
               ...
               if (($estadio_viejo== $cue_numero)&&($aforo_viejo==$tipo)&&($ciudad_vieja==$banco) && ($fecha_vieja == $fecha))    
				...
				else 
				{
					...
					mysql_query("UPDATE `flota` SET  `flo_nombre` =  '$cue_numero',  `flo_capacidad` =  '$fecha', `flo_cantidad` =  '$tipo', `fk_aer_id` =  '$banco' WHERE  `flota`.`flo_id` = $cue_id");
					...
					{//1
					   ...
					   $result1= mysql_query("SELECT * FROM flota f WHERE f.flo_id ='$cue_numero' AND f.fk_aer_id = '$banco'");
		  ...
		  {//aqui pinta el editor por primera vez	
					...
					$panelestadios = new Panel("../html/a_crear_flota_aerolinea.html");
					$panelestadios->add("form",'<form name="form1" method="post" action="../php/a_editar_flota_aerolinea.php?cue_id='.$id.'">');
