<?php
	...
	$panelcuentas = new Panel("../html/a_presupuesto_undestino_conestadia_aereo.html");
	$panelcuentas->add("crear",'<a href="../php/a_crear_presupuesto_undestino_conestadia_aereo.php">Realizar un Presupuesto</a>');
	
	

	$result= mysql_query("SELECT p.pre_id,p.pre_fecha,a.aer_nombre,d.des_nombre, o.des_nombre, ho.hot_nombre,p.pre_habitacion,s.ser_nombre,p.pre_total,p.pre_abono,p.pre_cant_per,i.via_id,f.via_id,ho.hot_id,p.fk_via_id_origen,p.fk_via_id_destino,p.fk_per_cedula,pe.per_nombre1,pe.per_apellido1 FROM via i, via f, destino d, destino o, aerolinea a, hotel ho, presupuesto p,servicio s, habitacion ha, persona pe WHERE p.fk_via_id_origen = i.via_id AND p.fk_via_id_destino = f.via_id AND i.fk_des_id = d.des_id AND f.fk_des_id = o.des_id and i.fk_aer_id=a.aer_id AND p.pre_habitacion=ha.hab_id AND ha.fk_hot_id=ho.hot_id AND p.pre_servicio=s.ser_id AND p.pre_status='No comprado' and p.pre_paseo IS NULL and p.fk_per_cedula=pe.per_cedula ORDER BY p.pre_id");
    ...
    <tr align="center">
	  ...
	  <td width="300"><a href="../php/a_comprar_undestino_conestadia_aereo.php?id='.$row['pre_id'].'&fecha='.$row['pre_fecha'].'&aerolinea='.$row[2].'&origen='.$row[3].'&destino='.$row[4].'&hotel='.$row['hot_nombre'].'&habitacion='.$row['pre_habitacion'].'&servicio='.$row['ser_nombre'].'&paseo='.$row['pas_nombre'].'&total='.$row['pre_total'].'&cantper='.$row['pre_cant_per'].'&origen1='.$row[11].'&destino1='.$row[12].'&hotel1='.$row[13].'&cedula='.$row[16].'">Comprar</a><a href="../php/a_eliminar_presupuesto_undestino_conestadia_aereo.php?id='.$row["pre_id"].'" onclick="return confirmar()">  Eliminar</a></td></tr>';
