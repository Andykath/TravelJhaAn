<?php
	...
	extract($_POST);
	      ...
	      if($estadio_viejo)
		  {
		     ...
		     $result1= mysql_query("SELECT * FROM via v WHERE v.via_terminal ='$cue_numero'");
			  ...
			  if (mysql_fetch_array($result1))
			  {
					...
					$panelcuentas = new Panel("../html/a_crear_costo1_crucero.html");
					$panelcuentas->add("form",'<form name="form1" method="post" action="../php/a_crear_costo_crucero.php">');
					...
					$select='';
					$result= mysql_query("SELECT * FROM  crucero a");
	      ...
	      else
		  {
			...
			$admin->add("body",'<body onLoad = "actual(1)" >');
			$panelcuentas = new Panel("../html/a_crear_costo1_crucero.html");
			$panelcuentas->add("form",'<form name="form1" method="post" action="../php/a_crear_costo1_crucero.php">');
		   ...
		   if ($selected && $fecha && $cue_numero)
		   {
			   ...
			   $res=mysql_query("SELECT fk_cru_id FROM  via where via_id=$selected");
			    ...
			    $select3='';
	        $result3= mysql_query("SELECT v.*, a.cru_nombre, d.des_nombre FROM  via v, crucero a, destino d WHERE v.fk_des_id=d.des_id AND v.fk_cru_id=a.cru_id and v.fk_cru_id=$devuelve AND v.fk_des_id<>$devuelve1 AND v.fk_aer_id IS NULL AND v.fk_ter_id IS NULL");
