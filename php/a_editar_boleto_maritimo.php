<?php
	...
	extract($_POST);
	      ...
	      if($viaje)
		  {
				...
				if ($res>0)
				{
					$hola='a_boleto_maritimo.php?mensaje=4';
		  ...
		  {			
			...
			$panelcuentas = new Panel("../html/a_editar_boleto_maritimo.html");
			$panelcuentas->add("form",'<form name="form1" method="post" action="../php/a_editar_boleto_maritimo.php">');
			$panelcuentas->add("id",$id);
			$panelcuentas->add("aqui",'<a href="../php/a_boleto_maritimo.php">Volver<<</a>');
			...
			if ($mensaje==1)
			 {
				...
				$select='';
	        	 $result= mysql_query("SELECT v. * FROM viaje v, via i, crucero a, persona p, presupuesto o, acompanante m
WHERE v.fk_via_id_destino = i.via_id AND i.fk_cru_id = a.cru_id AND v.fk_pre_id = o.pre_id AND o.fk_per_cedula=m.fk_per_cedula AND p.per_cedula = o.fk_per_cedula AND m.fk_per_cedula = p.per_cedula");
				...
				echo "cliente".$persona;
				 $select='';
	        	 $result= mysql_query("SELECT v.* FROM  viaje v, via i, crucero a, persona p, presupuesto o where v.fk_via_id_destino=i.via_id and i.fk_cru_id=a.cru_id and v.fk_pre_id=o.pre_id and o.fk_per_cedula='$persona' AND p.per_cedula = o.fk_per_cedula");
				 while($row = mysql_fetch_array($result))
        		{
					if ($row["via_id"]==$via)
					{
					...
					$tipo=$row["via_tipoviaje"];
						...
						$query=mysql_query("SELECT a.cru_nombre, c.des_nombre as ciudad, p.des_nombre as pais from via v, crucero a, destino c, destino p where v.via_id='$des' and a.cru_id=v.fk_cru_id and v.fk_des_id=c.des_id and c.fk_des_id=p.des_id") or die (mysql_error());
			...
			{	$panelcuentas->add("id",$id);
				...
				$panelcuentas->add("dueños",$select2);
	        	 ...
	        	 $result= mysql_query("SELECT v.* FROM  viaje v, via i, crucero a, persona p, presupuesto o where v.fk_via_id_destino=i.via_id and i.fk_cru_id=a.cru_id and v.fk_pre_id=o.pre_id and o.fk_per_cedula='$dueño' AND p.per_cedula = o.fk_per_cedula") or die (mysql_error());
				 while($row0 = mysql_fetch_array($result))
        		{
					
					$select_actual='<option value="'.$row0["via_id"].'">'.$row0["via_id"].'</option>'; 		
					$select=$select.$select_actual;
				}
				$panelcuentas->add("viajes",$select);
				}
...
WHERE v.fk_via_id_destino = i.via_id AND i.fk_cru_id = a.cru_id AND v.fk_pre_id = o.pre_id AND o.fk_per_cedula=m.fk_per_cedula AND p.per_cedula = o.fk_per_cedula AND m.fk_per_cedula = p.per_cedula");
			...
			if ($selected && $dueño && $id)
				...
				$panelcuentas->add("dueños",$select2);
					 ...
					 $select='';
	        	 $result= mysql_query("SELECT v.* FROM  viaje v, via i, crucero a, persona p, presupuesto o where v.fk_via_id_destino=i.via_id and i.fk_cru_id=a.cru_id and v.fk_pre_id=o.pre_id and o.fk_per_cedula='$dueño' AND p.per_cedula = o.fk_per_cedula");
				...
				if ($row3["via_id"]==$selected)
					{
						...
						$query=mysql_query("SELECT a.cru_nombre, c.des_nombre as ciudad, p.des_nombre as pais from via v, crucero a, destino c, destino p where v.via_id='$des' and a.cru_id=v.fk_cru_id and v.fk_des_id=c.des_id and c.fk_des_id=p.des_id");
