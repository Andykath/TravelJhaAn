<?php
	...
	extract($_POST);
		  ...
		  {// aqui validar que los datos esten bien y hacer update
               ...
               if (($estadio_viejo== $cue_numero)&&($aforo_viejo==$tipo)&&($ciudad_vieja==$banco) && ($fecha_vieja == $fecha))    
				...
				else 
				{
					...
					//echo "auqi";
					mysql_query("UPDATE `hotel` SET  `hot_nombre` =  '$cue_numero', `hot_estrellas` =  '$tipo', `hot_telefono` =  '$fecha',`fk_des_id` =  '$banco' WHERE  `hotel`.`hot_id` = $cue_id");
					...
					{//1
					   ...
					   $result1= mysql_query("SELECT * FROM hotel h WHERE h.hot_nombre ='$cue_numero' AND h.fk_des_id = '$banco'");
					  if (mysql_fetch_array($result1))
					  {
					  		$hola='a_hoteles.php?mensaje=3';
				            header("Location:$hola");		
					  ...
					  {
					 // echo "$cue_id, $cue_numero, .$tipo., $fecha , $banco";
					  mysql_query("UPDATE `hotel` SET  `hot_nombre` =  '$cue_numero', `hot_estrellas` =  '$tipo', `hot_telefono` =  '$fecha',`fk_des_id` =  '$banco' WHERE  `hotel`.`hot_id` = $cue_id");
		  ...
		  {//aqui pinta el editor por primera vez	
					...
					$panelestadios = new Panel("../html/a_crear_hotel.html");
					$panelestadios->add("form",'<form name="form1" method="post" action="../php/a_modificar_hotel.php?cue_id='.$id.'">');
					...
					$select='';
					$result= mysql_query("SELECT * FROM  destino where des_descripcion='Ciudad'");
					while($row = mysql_fetch_array($result))
					{				
