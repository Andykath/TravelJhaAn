<?php
	...
	$panelcuentas->add("crear",'<a href="../php/a_crear_boleto_aereo.php?mensaje=1">Crear Boleto</a>');
	...
	$result= mysql_query("SELECT b. * , v.via_id, v.fk_via_id_origen, v.fk_via_id_destino, e.aer_nombre FROM viaje v, boleto b, via a, aerolinea e WHERE b.fk_via_id = v.via_id AND a.fk_aer_id = e.aer_id AND v.fk_via_id_origen = a.via_id");
    ...
    <tr align="center">
	  ...
	  <td width="242"><div align="center">'.$cliente.'</td>
	   <td width="136"><div align="center">'.$row["aer_nombre"].'</td>
	   <td width="136"><div align="center">'.$row2["ciudad"].", ".$row2["pais"].'</td>
	   <td width="136"><div align="center">'.$row5["ciudad"].", ".$row5["pais"].'</td>
	  <td width="200"><a href="../php/a_editar_boleto_aereo.php?id='.$row['bol_id'].'&cliente='.$row['fk_per_cedula'].'&aco='.$row['fk_aco_id'].'&via='.$id.'&mensaje=1">Modificar</a> <a href="../php/a_eliminar_boleto_aereo.php?cue_id='.$row["bol_id"].'" onclick="return confirmar()">Eliminar</a></td>
    </tr>';
