<?php
	...
	//echo($locura);
     ...
     //echo("entrooooo");
						...
						$reshola=mysql_query("SELECT flo_id FROM  flota f , viaje v where  f.fk_ter_id='$aerid1'");
			  ...
			  {
				    $hola="En familia";}
					//echo("$hola,$fecha,$fecha1,$cantper,$id,$viaorigen,$viadestino,$flota1");
...
//echo($viaje);
		  ...
		  {//aqui pinta el editor por primera vez	
					...
					$panelestadios = new Panel("../html/u_presupuesto_comprar_undestino_sinestadia_terrestre.html");
					$panelestadios->add("form",'<form name="form1" method="post" action="../php/u_comprar_undestino_sinestadia_terrestre.php?id='.$id.'&aerolinea='.$aerolinea.'&origen='.$origen.'&destino='.$destino.'&total='.$total.'&cantper='.$cantper.'&tipo='.$tipo.'&fechapago='.$fechapago.'&numero='.$numero.'&combo='.$combo.'&nombre='.$nombre.'&cvv2='.$cvv2.'&monto='.$monto.'&monto1='.$monto1.'&monto2='.$monto2.'">');
