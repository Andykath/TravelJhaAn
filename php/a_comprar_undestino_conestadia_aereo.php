<?php
	...
	////echo($locura);
		...
		if($formapago=="Pago Unico")
		{
				   ...
				   if($cantper>2)
			  {
					...
					////echo($hotel);
				       $res3=mysql_query("SELECT hot_id FROM  hotel where hot_nombre='$hotel'");
						...
						$reshola=mysql_query("SELECT flo_id FROM  flota f  where  f.fk_aer_id='$aerid1' and (f.flo_actual>0)");
			  ...
			  {
				    $hola="En familia";}
					//echo("$hola,$fecha,$fecha1,$cantper,$id,$viaorigen,$viadestino,$flota1,hola1");
...
////echo($viaje);
			...
			////echo($pago);
					...
					//$panelestadios->add("continuar",'<a href="../php/a_continuar_compra_undestino_conestadia_aereo.php?viaje='.$viaje.'&cedula='.$cedula.'&cantper='.$cantper.'">Continuar</a>');
				...
				$hola='a_continuar_compra_undestino_conestadia_aereo.php?viaje='.$viaje.'&cantper='.$cantper.'&cedula='.$cedula.'&mensaje=2&preg='.$preg.'';
		...
		//	//echo($pago2);
			////echo("pago2");
					...
					mysql_query("UPDATE `presupuesto` SET  `pre_status` = 'Comprado' WHERE  `presupuesto`.`pre_id` = $id");
					   ...
					   $hola1='a_continuar_compra_undestino_conestadia_aereo.php?viaje='.$viaje.'&cantper='.$cantper.'&cedula='.$cedula.'&mensaje=2&preg='.$preg.'';
...
mysql_query("INSERT INTO `estadia` (`est_id`,`est_fecha_ini`,`est_fecha_fin`,`fk_via_id`,`fk_hab_id`) VALUES(NULL,'$fecha','$fecha1','$viaje','$habitacion')");
		    ...
		    $pago2=$roc['max'];
					 ...
					 $antes=mysql_query("SELECT `pre_abono` FROM  presupuesto WHERE pre_id='$id'");
						 ...
						 mysql_query("UPDATE `presupuesto` SET  `pre_abono` = ($antes1+$montoaux+$montoaux1+$montoaux2) WHERE  `presupuesto`.`pre_id` = $id");
						   ...
						   $hola1='a_presupuesto_undestino_conestadia_aereo1.php?mensaje=2';
				           header("Location:$hola1");
					   ...
					   mysql_query("INSERT INTO `tip_pag` (`tip_id`,`fk_pag_id`,`fk_tar_id`,`fk_che_id`,`fk_dep_id`,`fk_mil_id`) VALUES(NULL,'$pago2',NULL,NULL,'$deposito',NULL)");
						 ...
						 $antes=mysql_query("SELECT `pre_abono` FROM  presupuesto WHERE pre_id='$id'");
		                     ...
		                     $antes1=$antesb['pre_abono'];
							  ...
							  mysql_query("UPDATE `presupuesto` SET  `pre_abono` = ($antes1+$montoaux2) WHERE  `presupuesto`.`pre_id` = $id"); $hola1='a_presupuesto_undestino_conestadia_aereo1.php?mensaje=2';
				           header("Location:$hola1");
						...
						{
						 $hola1='u_monto_error_conestadia_aereo.php?mensaje=2';
						header("Location:$hola1");
						}
			
		
			}// del else si inserto es 0 
...

		{
			...
			$hola1='a_flota_error_conestadia_aereo.php?mensaje=2';
		  ...
		  {//aqui pinta el editor por primera vez	
					...
					$panelestadios = new Panel("../html/a_presupuesto_comprar_undestino_conestadia_aereo.html");
					$panelestadios->add("form",'<form name="form1" method="post" action="../php/a_comprar_undestino_conestadia_aereo.php?id='.$id.'&aerolinea='.$aerolinea.'&origen='.$origen.'&destino='.$destino.'&hotel='.$hotel.'&habitacion='.$habitacion.'&servicio='.$servicio.'&paseo='.$paseo.'&total='.$total.'&cantper='.$cantper.'&tipo='.$tipo.'&fechapago='.$fechapago.'&numero='.$numero.'&combo='.$combo.'&nombre='.$nombre.'&cvv2='.$cvv2.'&monto='.$monto.'&monto1='.$monto1.'&monto2='.$monto2.'&fecha='.$fechai.'&fecha='.$fechaf.'">');
...
AND g.des_nombre='$destino')");
    ...
    <th align="left" scope="row"><div align="left" id="totalex">'.$total1/$rowok2["mon_descripcion"].'$</div></th>
				...
				$panelestadios->add("pago",' <tr><td>Tipo:</td><td><select name="tipo" id="tipo"  onChange="populate2(document.form1,document.form1.formapago.options[document.form1.formapago.selectedIndex].value,document.form1.fecha.value,document.form1.fecha1.value,document.getElementById(\'aerolinea\').innerHTML,document.getElementById(\'origen\').innerHTML,document.getElementById(\'destino\').innerHTML,document.getElementById(\'hotel\').innerHTML,document.getElementById(\'habitacion\').innerHTML,document.getElementById(\'servicio\').innerHTML,document.getElementById(\'total\').innerHTML,document.getElementById(\'cantper\').innerHTML,document.form1.tipo.options[document.form1.tipo.selectedIndex].value,document.getElementById(\'id\').innerHTML,document.getElementById(\'cedula\').innerHTML)"><option selected="selected">Seleccione</option><option>Tarjeta</option><option>Cheque</option><option>Efectivo</option></select></td></tr>
    ');
