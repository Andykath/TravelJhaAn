<?php
	...
	extract($_POST);
		  ...
		  {// aqui validar que los datos esten bien y hacer update
               ...
               if (($estadio_viejo== $cue_numero)&&($aforo_viejo==$tipo)&&($ciudad_vieja==$banco) && ($fecha_vieja == $fecha))    
				...
				else 
				{
					...
					mysql_query("UPDATE `crucero` SET  `cru_nombre` =  '$cue_numero',`cru_fecha_inicio` =  '$fecha',`fk_des_id` =  '$banco' WHERE  `crucero`.`cru_id` = $cue_id");
					...
					{//1
					   ...
					   $result1= mysql_query("SELECT * FROM crucero c WHERE c.cru_id ='$cue_numero' AND c.fk_des_id = '$banco'");
		  ...
		  {//aqui pinta el editor por primera vez	
					...
					$panelestadios = new Panel("../html/a_crear_crucero.html");
					$panelestadios->add("form",'<form name="form1" method="post" action="../php/a_editar_crucero.php?cue_id='.$id.'">');
