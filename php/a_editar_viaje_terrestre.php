   ...
   $cedula=$_SESSION['cedula'];
	...
	extract($_GET);
		  ...
		  if($fecha)
		  {
			...
			$result1= mysql_query("SELECT a.ter_nombre, a.ter_id FROM  via v, terrestre a, destino d WHERE v.fk_des_id=d.des_id AND v.fk_ter_id=a.ter_id AND v.fk_ter_id=$devuelve AND v.fk_des_id<>$devuelve1 AND v.fk_cru_id IS NULL AND v.fk_aer_id IS NULL") or die (mysql_error());
			$row=mysql_fetch_array($result1);
			$aer=$row["ter_id"];
			$result2= mysql_query("SELECT flo_id from flota where fk_ter_id='$aer'");
	      ...
	      else
		  {
			...
			$panelcuentas = new Panel("../html/a_editar_viaje_terrestre.html");
			$panelcuentas->add("form",'<form name="form1" method="post" action="../php/a_editar_viaje_terrestre.php?">');
			 $panelcuentas->add("Aqui",'<a href="../php/a_viajes_terrestre.php">Volver<<</a>');
	...
	// 	selected=2&fechai=2011-12-19&fecha22011-12-21&selected2=k&id=4
			...
			//  selected=12&fechai=2011-06-28&selected2=14&fecha2=2011-06-21&id=20
