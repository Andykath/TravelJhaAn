		...
		//echo "$id1 , $id2";
               if (($nombre_1== $nombre1)&&($nombre_2==$nombre2)&&($apellido_1==$apellido1)&&($apellido_2==$apellido2)&&($sexo1==$sexo)&&($direccion1==$direccion)&&($fecha_vieja==$fecha)&&($edo_civil1==$edocivil)&&($nacionalidad1==$nacionalidad)&&($visa1==$visa)&&($pasaporte1==$pasaporte))    
				...
				else 
				{
					...
					$result3= mysql_query("SELECT per_tipo FROM persona WHERE per_cedula=$cedula");
			  ...
			  $nacionalidad1=$row[0];
					...
					{//1
					$result4= mysql_query("SELECT per_tipo FROM persona WHERE per_cedula=$cedula1");
			        ...
			        $nacionalidad2=$row1[0];
					  mysql_query("UPDATE `persona` SET  `per_cedula` =  '$cedula', `per_nombre1` =  '$nombre1',`per_nombre2` =  '$nombre2', `per_apellido1`='$apellido1',`per_apellido2` =  '$apellido2',`per_sexo` =  '$sexo',`per_tipo` =  '$tipo2',`per_direccion` =  '$direccion',`per_fecha_nac` =  '$fecha',`per_edo_civil` =  '$edocivil',`per_nacionalidad` =  '$nacionalidad2',`per_visa` =  '$visa',`per_pasaporte` =  '$pasaporte' WHERE  `persona`.`per_cedula` = $cedula1");
		  ...
		  {//aqui pinta el editor por primera vez	
					...
					$panelestadios->add("form",'<form name="form1" method="post" action="../php/u_modificar_usuario.php?nombre1='.$nombre1.'&nombre2='.$nombre2.'&apellido1='.$apellido1.'&apellido2='.$apellido2.'&cedula='.$cedula.'&direccion='.$direccion.'&fechanac='.$fechanac.'&sexo='.$sexo.'&edocivil='.$edocivil.'&pasaporte='.$pasaporte.'&visa='.$visa.'&login='.$login.'&password='.$password.'&nacionalidad='.$nacionalidad.'">');
