<?php
	...
	$panelmiperfil = new Panel("../html/a_verperfil_promocion_estadia_cruceros.html");
   ...
   $cedula_actual=$_SESSION['cedula'];
    ...
    $panelmiperfil->add("aernombre1",$aernombre1);
  ...
  //$panelmiperfil->add("modif",'<a href="../php/u_telefonos.php?&cedula='.$row['per_cedula'].'">Gestionar</a>');
   ...
   $result6=mysql_query("SELECT ho.flo_nombre , ho.flo_id FROM flota ho WHERE ho.flo_id='$fkhab'");
	$row6 = mysql_fetch_array($result6);
                       $panelmiperfil->add("aernombre",$row6['flo_nombre']);
		               

	
	$panelmiperfil->add("links",'<a href="../php/a_modificar_promocion_estadia_crucero.php?nombre='.$nombrepro.'&id='.$id.'&fechai='.$fechai.'&fechaf='.$fechaf.'&descuento='.$descuento.'&duracion='.$duracion.'&fkvia='.$fkvia.'&fkviaid='.$fkviaid.'&hotel='.$row6['flo_id'].'&habitacion='.$row6['hab_id'].'">Modificar</a> <a href="../php/a_eliminar_promocion_estadia_crucero.php?idpro='.$id.'" onclick="return confirmar()">Eliminar</a>');
