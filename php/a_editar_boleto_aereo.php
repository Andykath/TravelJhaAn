<?php
	...
	extract($_POST);
		  ...
		  {			
			...
			$panelcuentas = new Panel("../html/a_ediat_boleto_aereo.html");
			$panelcuentas->add("form",'<form name="form1" method="post" action="../php/a_editar_boleto_aereo.php">');
			$panelcuentas->add("id",$id);
			$panelcuentas->add("aqui",'<a href="../php/a_boleto_aereo.php">Volver<<</a>');
			...
			if ($mensaje==1)
			 {
				...
				$select='';
	        	 $result= mysql_query("SELECT v. * FROM viaje v, via i, aerolinea a, persona p, presupuesto o, acompanante m
WHERE v.fk_via_id_destino = i.via_id AND i.fk_aer_id = a.aer_id AND v.fk_pre_id = o.pre_id AND o.fk_per_cedula=m.fk_per_cedula AND p.per_cedula = o.fk_per_cedula AND m.fk_per_cedula = p.per_cedula");
				...
				echo "cliente".$persona;
				 $select='';
	        	 $result= mysql_query("SELECT v.* FROM  viaje v, via i, aerolinea a, persona p, presupuesto o where v.fk_via_id_destino=i.via_id and i.fk_aer_id=a.aer_id and v.fk_pre_id=o.pre_id and o.fk_per_cedula='$persona' AND p.per_cedula = o.fk_per_cedula");
				 while($row = mysql_fetch_array($result))
        		{
					if ($row["via_id"]==$via)
					{
