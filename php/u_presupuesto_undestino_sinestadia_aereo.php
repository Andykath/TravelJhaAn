<?php
	...
	$panelcuentas = new Panel("../html/u_presupuesto_undestino_sinestadia_aereo.html");
	$panelcuentas->add("crear",'<a href="../php/u_crear_presupuesto_undestino_sinestadia_aereo.php">Realizar un Presupuesto</a>');
	
	

	$result= mysql_query("SELECT p.pre_id,p.pre_fecha,a.aer_nombre,d.des_nombre, o.des_nombre,p.pre_total,p.pre_abono,p.pre_cant_per,i.via_id,f.via_id FROM via i, via f, destino d, destino o, aerolinea a, presupuesto p WHERE p.fk_via_id_origen = i.via_id AND p.fk_via_id_destino = f.via_id AND i.fk_des_id = d.des_id AND f.fk_des_id = o.des_id and i.fk_aer_id=a.aer_id AND p.fk_per_cedula='$cedula' AND p.pre_status='No comprado' AND p.pre_habitacion IS NULL ORDER BY p.pre_id");
