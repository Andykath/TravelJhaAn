	...
	extract($_GET);
		  ...
		  if($fecha)
		  {
			...
			//echo("entra  $total $total");
				...
				$hola='a_presupuesto_undestino_sinestadia_aereo.php?mensaje=1';
	      ...
	      else
		  {
			...
			$panelcuentas = new Panel("../html/a_crear_presupuesto_undestino_sinestadia_aereo.html");
			$panelcuentas->add("form",'<form name="form1" method="post" action="../php/a_crear_presupuesto_undestino_sinestadia_aereo.php?">');
        ...
        <option>Ida y vuelta</option>');
					 ...
					 $selecty='';
	        $resulty= mysql_query("SELECT p.per_cedula,p.per_nombre1,p.per_apellido1 FROM persona p ");
        	while($rowy = mysql_fetch_array($resulty))
        	{
			$select_actualy='<option value="'.$rowy["per_cedula"].'">'.$rowy["per_cedula"]." ".$rowy["per_nombre1"]." ".$rowy["per_apellido1"].'</option>'; 		
			$selecty=$selecty.$select_actualy;
			}
			$panelcuentas->add("cedulas",$selecty);
