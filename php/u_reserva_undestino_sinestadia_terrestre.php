<?php
	...
	$panelcuentas = new Panel("../html/u_reserva_undestino_sinestadia_terrestre.html");
	
	
	

	$result= mysql_query("SELECT  v.via_id,v.via_fecha_ini,a.ter_nombre,d.des_nombre, o.des_nombre,p.pre_total,p.pre_abono,p.pre_cant_per,i.via_id,f.via_id,p.pre_id FROM via i, via f, destino d, destino o, terrestre a, presupuesto p, viaje v WHERE p.fk_via_id_origen = i.via_id AND p.fk_via_id_destino = f.via_id AND i.fk_des_id = d.des_id AND f.fk_des_id = o.des_id and i.fk_ter_id=a.ter_id AND p.fk_per_cedula='$cedula' AND p.pre_status='No comprado' and v.fk_pre_id=p.pre_id and v.via_tipo='Reserva' and p.pre_paseo IS NULL  and p.pre_habitacion is null ORDER BY v.via_id");
    ...
    <tr align="center">
	  ...
	  <td width="200"><a href="../php/u_comprarR_undestino_sinestadia_terrestre.php?id='.$row['pre_id'].'&fecha='.$row['pre_fecha'].'&aerolinea='.$row['ter_nombre'].'&origen='.$row[3].'&destino='.$row[4].'&total='.$row['pre_total'].'&cantper='.$row['pre_cant_per'].'&origen1='.$row[11].'&destino1='.$row[12].'&hotel1='.$row[13].'&id2='.$row['via_id'].'">Comprar</a> </td>
    </tr>';
