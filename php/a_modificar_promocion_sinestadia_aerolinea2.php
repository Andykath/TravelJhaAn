<?php
	...
	extract($_GET);
	      ...
	      if($cue_numero)
		  {
				 ...
				 mysql_query("UPDATE `promocion` SET  `pro_nombre` =  '$cue_numero', `pro_fechaini` =  '$fecha',`pro_fechafin` =  '$fecha1', `pro_descuento` =  '$des',`pro_durac_viaje` =  '$dur',`fk_via_id` =  '$banco',`fk_via_id2` =  '$banco1' WHERE  `promocion`.`pro_id` = $id");
	      ...
	      else // aqui cargo por primera vez 
		  {
			...
			$panelcuentas = new Panel("../html/a_modificar_promocion_sinestadia_aerolinea.html");
			$panelcuentas->add("form",'<form name="form1" method="post" action="../php/a_modificar_promocion_sinestadia_aerolinea2.php">');
