<?php
	...
	$panelmiperfil = new Panel("../html/u_miperfil.html");
   ...
   $cedula_actual=$_SESSION['cedula'];
	...
	$panelmiperfil->add("links",'<a href="../php/u_modificar_usuario.php?nombre1='.$row['per_nombre1'].'&nombre2='.$row['per_nombre2'].'&apellido1='.$row['per_apellido1'].'&apellido2='.$row['per_apellido2'].'&cedula='.$row['per_cedula'].'&direccion='.$row['per_direccion'].'&fechanac='.$row['per_fecha_nac'].'&sexo='.$row['per_sexo'].'&edocivil='.$row['per_aedo_civil'].'&pasaporte='.$row['per_pasaporte'].'&visa='.$row['per_visa'].'&login='.$row['usu_login'].'&password='.$row['usu_password'].'&nacionalidad='.$row['per_nacionalidad'].'">Modificar</a> <a href="../php/u_eliminar_usuario.php?cedula='.$row["per_cedula"].'" onclick="return confirmar()">Eliminar</a>');
   
   
   $panelmiperfil->add("modif",'<a href="../php/u_telefonos.php?&cedula='.$row['per_cedula'].'">Gestionar</a>');
		
   $result6=mysql_query("SELECT t.tel_numero,t.tel_tipo,t.tel_id FROM telefono t WHERE t.fk_per_cedula='$cedula_actual'");
