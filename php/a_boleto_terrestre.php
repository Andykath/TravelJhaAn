<?php
	...
	$panelcuentas = new Panel("../html/a_boleto_terrestre.html");
	$panelcuentas->add("crear",'<a href="../php/a_crear_boleto_terrestre.php?mensaje=1">Crear Boleto</a>');
	...
	$result= mysql_query("SELECT b. * , v.via_id, v.fk_via_id_origen, v.fk_via_id_destino, e.ter_nombre FROM viaje v, boleto b, via a, terrestre e WHERE b.fk_via_id = v.via_id AND a.fk_ter_id = e.ter_id AND v.fk_via_id_origen = a.via_id");
    ...
    <tr align="center">
	  ...
	  <td width="242"><div align="center">'.$cliente.'</td>
	   <td width="136"><div align="center">'.$row["ter_nombre"].'</td>
	   <td width="136"><div align="center">'.$row2["ciudad"].", ".$row2["pais"].'</td>
	   <td width="136"><div align="center">'.$row5["ciudad"].", ".$row5["pais"].'</td>
	  <td width="200"><a href="../php/a_editar_boleto_terrestre.php?id='.$row['bol_id'].'&cliente='.$row['fk_per_cedula'].'&aco='.$row['fk_aco_id'].'&via='.$id.'&mensaje=1">Modificar</a> <a href="../php/a_eliminar_boleto_terrestre.php?cue_id='.$row["bol_id"].'" onclick="return confirmar()">Eliminar</a></td>
    </tr>';
