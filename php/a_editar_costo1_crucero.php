<?php
	...
	extract($_POST);
               ...
               if (($estadio_viejo== $cue_numero)&&($aforo_viejo==$tipo)&&($ciudad_vieja==$banco) && ($fecha_vieja == $fecha)&& ($destino_viejo==$destino))    
				...
				else 
				{
					...
					{//1
					   ...
					   $result1= mysql_query("SELECT * FROM via v WHERE v.via_id ='$cue_numero' AND v.fk_cru_id = '$banco' AND v.fk_des_id = '$destino'");
		  ...
		  {//aqui pinta el editor por primera vez	
					...
					$panelestadios = new Panel("../html/a_editar_costo1_crucero.html");
					$panelestadios->add("form",'<form name="form1" method="post" action="../php/a_editar_costo1_crucero.php?cue_id='.$id.'">');
					...
					$result2= mysql_query("SELECT v.*, a.cru_nombre, d.des_nombre FROM  via v, crucero a, destino d WHERE v.fk_des_id=d.des_id AND v.fk_cru_id=a.cru_id AND v.fk_cru_id IS NOT NULL AND v.fk_aer_id IS NULL AND v.fk_ter_id IS NULL");
								...
								$result21= mysql_query("SELECT v.*, a.cru_nombre, d.des_nombre FROM  via v, crucero a, destino d WHERE v.fk_des_id=d.des_id AND v.fk_cru_id=a.cru_id AND v.fk_cru_id IS NOT NULL AND v.fk_aer_id IS NULL AND v.fk_ter_id IS NULL");
								while($row21 = mysql_fetch_array($result21))
								{				
									
									if ($row21["via_id"]==$destino){
										//echo 'if';
										$select_actual21='<option selected="selected" value="'.$row21["via_id"].'">'.$row21["cru_nombre"]."/".$row21["des_nombre"].'</option>'; }	
									else{
										//echo "banco es $banco";
										$select_actual21='<option value="'.$row21["via_id"].'">'.$row21["cru_nombre"]."/".$row21["des_nombre"].'</option>'; 	}	
