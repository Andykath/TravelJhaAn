<?php
	...
	//echo($locura);
		...
		$reschaot=mysql_query("SELECT ter_id FROM terrestre where ter_nombre='$aerolinea'");
						...
						$resholat=mysql_query("SELECT flo_actual FROM  flota f  where  f.fk_aer_id='$aerid10' and flo_actual>0");
		...
		if($formapago=="Pago Unico")
		{
				   ...
				   if($cantper>2)
			  {
					...
					//echo($hotel);
						...
						$reshola=mysql_query("SELECT flo_id FROM  flota f , viaje v where  f.fk_ter_id='$aerid1'  and flo_actual>0");
  ...
  $millas=mysql_query("SELECT via_millas FROM  via v where  via_id='$viadestino'");
		    ...
		    $pago2=$roc['max'];
					 ...
					 $antes=mysql_query("SELECT `pre_abono` FROM  presupuesto WHERE pre_id='$id'");
						 ...
						 $hola1='u_reserva_undestino_sinestadia_terrestre1.php?mensaje=2';
				           header("Location:$hola1");
		...
		{
			$hola1='u_flota_errorR_sinestadia_terrestre.php?mensaje=2';
						header("Location:$hola1");
			}
		  ...
		  {//aqui pinta el editor por primera vez	
					...
					$admin= new Panel("../html/usuario.html");
					$admin->add("body",'<body onLoad = "actual(3)" >');
					$panelestadios = new Panel("../html/u_reserva_comprar_undestino_sinestadia_terrestre.html");
					$panelestadios->add("form",'<form name="form1" method="post" action="../php/u_comprarR_undestino_sinestadia_terrestre.php?id='.$id.'&aerolinea='.$aerolinea.'&origen='.$origen.'&destino='.$destino.'&total='.$total.'&cantper='.$cantper.'&tipo='.$tipo.'&fechapago='.$fechapago.'&numero='.$numero.'&combo='.$combo.'&nombre='.$nombre.'&cvv2='.$cvv2.'&monto='.$monto.'&monto1='.$monto1.'&monto2='.$monto2.'&fecha='.$fechai.'&fecha1='.$fechaf.'">');
