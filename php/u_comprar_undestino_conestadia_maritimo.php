			...
			$cantperaux=$_POST['cantper'];
				   ...
				   if($cantper>2)
			  {
					...
					//echo "$hotel1 hotel1";
						...
						$reshola=mysql_query("SELECT flo_id FROM  flota f  where  f.fk_cru_id='$aerid1'");
...
//echo($viaje);
			...
			//echo($pago);
					...
					if($tipo=="Cheque")
					{
						mysql_query("INSERT INTO `cheque` (`che_id`,`che_num`,`che_cuenta`,`che_nombre`,`che_fechaven`, `fk_per_cedula`,`fk_ban_id`) VALUES(NULL,'$numeroaux','$cvv2aux','$nombreaux','$fechapagoaux','$cedula','$comboaux')");
					...
					if($tipo=="Efectivo")
					{
						mysql_query("INSERT INTO `deposito` (`dep_id`,`dep_numero`,`dep_cuenta`,`dep_fecha`,`fk_per_cedula`,`fk_ban_id`) VALUES(NULL,'$numeroaux','$cvv2aux','$fechapagoaux','$cedula','$comboaux')");	
	   ...
	   //echo($total);
		...
		if($inserto==1){
			...
			if($comboaux1 && $comboaux2 && $comboaux && $fechapagoaux && $fechapagoaux1 && $fechapagoaux2 && $numeroaux1 && $numeroaux2 && $numeroaux && $nombreaux && $nombreaux1 && $cvv2aux && $cvv2aux1 && $cvv2aux2 && $montoaux && $montoaux1 && $montoaux2) {
				 //echo("if");
                 $res3=mysql_query("SELECT hot_id FROM  flota where flo_nombre='$hotel'");
						$ro3 = mysql_fetch_array($res3);
						$hotel1=$ro3['flo_id'];	
						//echo "$hotel1 hotel1";
						...
						$reshola=mysql_query("SELECT flo_id FROM  flota f , viaje v where  f.fk_cru_id='$aerid1'");
						$rohola = mysql_fetch_array($reshola);
						$flota1=$rohola['flo_id'];	
			          //echo($flota1);
...
mysql_query("INSERT INTO `estadia` (`est_id`,`est_fecha_ini`,`est_fecha_fin`,`fk_via_id`,`fk_hab_id`) VALUES(NULL,'$fecha','$fecha1','$viaje','$habitacion')");
			...
			//echo("pago2");
					...
					mysql_query("UPDATE `presupuesto` SET  `pre_status` = 'Comprado' WHERE  `presupuesto`.`pre_id` = $id");
					   ...
					   $hola1='u_continuar_compra_undestino_conestadia_aereo.php?viaje='.$viaje.'&cantper='.$cantper.'&cedula='.$cedula.'&mensaje=2';
...
mysql_query("INSERT INTO `estadia` (`est_id`,`est_fecha_ini`,`est_fecha_fin`,`fk_via_id`,`fk_hab_id`) VALUES(NULL,'$fecha','$fecha1','$viaje','$habitacion')");
		  ...
		  {//aqui pinta el editor por primera vez	
					...
					$admin= new Panel("../html/usuario.html");
					$admin->add("body",'<body onLoad = "actual(3)" >');
					$panelestadios = new Panel("../html/u_presupuesto_comprar_undestino_conestadia_maritimo.html");
					$panelestadios->add("form",'<form name="form1" method="post" action="../php/u_comprar_undestino_conestadia_maritimo.php?id='.$id.'&aerolinea='.$aerolinea.'&origen='.$origen.'&destino='.$destino.'&hotel='.$hotel.'&habitacion='.$habitacion.'&servicio='.$servicio.'&paseo='.$paseo.'&total='.$total.'&cantper='.$cantper.'&tipo='.$tipo.'&fechapago='.$fechapago.'&numero='.$numero.'&combo='.$combo.'&nombre='.$nombre.'&cvv2='.$cvv2.'&monto='.$monto.'&monto1='.$monto1.'&monto2='.$monto2.'">');
