<?php
	...
	extract($_POST);
		  ...
		  {// aqui validar que los datos esten bien y hacer update
               ...
               if (($estadio_viejo== $cue_numero)&&($aforo_viejo==$tipo)&&($ciudad_vieja==$banco) && ($fecha_vieja == $fecha)&& ($destino_viejo==$destino))    
				...
				else 
				{
					...
					mysql_query("UPDATE  `via` SET  `via_costo` =  '$tipo',  `via_terminal` =  '$cue_numero', `via_millas` =  '$fecha', `fk_des_id` =  '$destino', `fk_cru_id` =  '$banco' WHERE  `via`.`via_id` = $cue_id");
		  ...
		  {//aqui pinta el editor por primera vez	
					...
					$panelestadios = new Panel("../html/a_crear_costo_crucero.html");
					$panelestadios->add("form",'<form name="form1" method="post" action="../php/a_editar_costo_crucero.php?cue_id='.$id.'">');
