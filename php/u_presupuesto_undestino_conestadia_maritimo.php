<?php
	...
	$panelcuentas->add("crear",'<a href="../php/u_crear_presupuesto_undestino_conestadia_maritimo.php?cedula='.$cedula.'">Realizar un Presupuesto</a>');
	
	

	$result= mysql_query("SELECT p.pre_id,p.pre_fecha,a.cru_nombre,d.des_nombre, o.des_nombre, ho.flo_nombre,p.pre_habitacion,s.ser_nombre,p.pre_total,p.pre_abono,p.pre_cant_per,i.via_id,f.via_id,ho.flo_id ,p.fk_per_cedula FROM via i, via f, destino d, destino o, crucero a, flota ho, presupuesto p,servicio s, habitacion ha WHERE p.fk_via_id_origen = i.via_id AND p.fk_via_id_destino = f.via_id AND i.fk_des_id = d.des_id AND f.fk_des_id = o.des_id and i.fk_cru_id=a.cru_id AND p.fk_per_cedula='$cedula' AND p.pre_habitacion=ha.hab_id AND ha.fk_cru_id=ho.flo_id AND p.pre_servicio=s.ser_id AND p.pre_status='No comprado' ORDER BY p.pre_id");
