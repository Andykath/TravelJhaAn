<?php
	...
	$panelcuentas = new Panel("../html/a_general_viaje_terrestre.html");
...
SELECT p.via_id FROM terrestre a, via v, viaje p WHERE p.fk_via_id_destino = v.via_id AND v.fk_ter_id = a.ter_id and p.via_tipo='Viaje')");
	...
	$destino= mysql_query("SELECT d.des_nombre AS pais, a.ter_id, o.des_nombre AS ciudad, a.ter_nombre FROM terrestre a, destino d, destino o, via v, viaje p WHERE p.fk_via_id_destino = v.via_id AND v.fk_des_id = o.des_id AND o.fk_des_id = d.des_id AND p.via_id ='$id' AND v.fk_ter_id=a.ter_id");
	  ...
	  <td width="96"><div align="center">'.$row3["ciudad"].', '.$row3["pais"].'</td>
	   ...
	   <td width="93"><div align="center">'.$row["via_millas"].'</td>
	    <td width="67"><div align="center">'.$row["via_cant_per"].'</td>
		 <td width="79"><div align="center">'.$row4["ter_nombre"].'</td>
	  <td width="200"><a href="../php/a_editar_viaje_terrestre.php?id='.$row['via_id'].'&origen='.$row3['ter_id'].'&via='.$row['fk_via_id_origen'].'&destino='.$row['fk_via_id_destino'].'&fecha_ini='.$row['via_fecha_ini'].'&fecha_fin='.$row['via_fecha_fin'].'&tipo='.$row['via_tipoviaje'].'&mensaje=1'.'&cant='.$row['via_cant_per'].'">Modificar</a> <a href="../php/a_eliminar_viaje_terrestre.php?cue_id='.$row["via_id"].'" onclick="return confirmar()">Eliminar</a></td>
    </tr>';
