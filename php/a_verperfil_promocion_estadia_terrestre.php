<?php
	...
	$panelmiperfil = new Panel("../html/a_verperfil_promocion_estadia_terrestre.html");
  ...
  //$panelmiperfil->add("modif",'<a href="../php/u_telefonos.php?&cedula='.$row['per_cedula'].'">Gestionar</a>');
   ...
   $result6=mysql_query("SELECT ho.hot_nombre , ho.hot_id FROM hotel ho WHERE ho.hot_id='$fkhab'");
	...
	$panelmiperfil->add("links",'<a href="../php/a_modificar_promocion_estadia_terrestre.php?nombre='.$nombrepro.'&id='.$id.'&fechai='.$fechai.'&fechaf='.$fechaf.'&descuento='.$descuento.'&duracion='.$duracion.'&fkvia='.$fkvia.'&fkviaid='.$fkviaid.'&hotel='.$row6['hot_id'].'&habitacion='.$row6['hab_id'].'">Modificar</a> <a href="../php/a_eliminar_promocion_estadia_terrestre.php?idpro='.$id.'" onclick="return confirmar()">Eliminar</a>');
