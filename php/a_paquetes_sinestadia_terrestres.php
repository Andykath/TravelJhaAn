<?php
	...
	$panelcuentas->add("crear",'<a href="../php/a_crear_paquete_sinestadia_terrestre.php">Crear Paquete </a>');
	...
	$result= mysql_query("SELECT v. * , d.des_nombre, o.des_nombre, a.ter_nombre, a.ter_id
...
and v.via_tipo='Paquete' ORDER BY v.via_id");
    ...
    <tr align="center">
	  ...
	  <td width="200"><a href="../php/a_verperfil_paquete_terrestre.php?tipoviaje='.$row['via_tipoviaje'].'&id='.$row['via_id'].'&fechai='.$row['via_fecha_ini'].'&fechaf='.$row['via_fecha_fin'].'&millas='.$row['via_millas'].'&tipopaq='.$row['via_tipo_paq'].'&cantpaq='.$row['via_cant_per'].'&aernombre='.$row['ter_nombre'].'&aerid='.$row['ter_id'].'&fkvia='.$row['fk_via_id_origen'].'&fkvia2='.$row['fk_via_id_destino'].'">Ver mas</a>
    </tr>';
