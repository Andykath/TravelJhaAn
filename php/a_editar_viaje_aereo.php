   ...
   $cedula=$_SESSION['cedula'];
	...
	extract($_GET);
		  ...
		  if($fecha)
		  {
			...
			$result1= mysql_query("SELECT a.aer_nombre, a.aer_id FROM  via v, aerolinea a, destino d WHERE v.fk_des_id=d.des_id AND v.fk_aer_id=a.aer_id AND v.fk_aer_id=$devuelve AND v.fk_des_id<>$devuelve1 AND v.fk_cru_id IS NULL AND v.fk_ter_id IS NULL")or die (mysql_error());
			$row=mysql_fetch_array($result1);
			$aer=$row["aer_id"];
			/*$result2= mysql_query("SELECT flo_id from flota where fk_aer_id='$aer'")or die (mysql_error());
			$row2=mysql_fetch_array($result2);
			$flota=$row2["flo_id"];*/
			mysql_query("UPDATE `viaje` SET  `via_fecha_ini`= '$fecha', `via_fecha_fin`= '$fecha2', `via_cant_per`='$cantper', `fk_via_id_origen`='$banco', `fk_via_id_destino`='$banco1', via_tipoviaje='$tipoviaje'  where via_id='$id'") or die (mysql_error());
				$hola='a_viajes.php?mensaje=2';
	      ...
	      else
		  {
			...
			$panelcuentas = new Panel("../html/a_editar_viaje_aereo.html");
			$panelcuentas->add("form",'<form name="form1" method="post" action="../php/a_editar_viaje_aereo.php?">');
			 $panelcuentas->add("Aqui",'<a href="../php/a_viajes.php">Volver<<</a>');
			...
			$res=mysql_query("SELECT fk_aer_id FROM  via where via_id=$via");
								$ro = mysql_fetch_array($res);
								$devuelve=$ro['fk_aer_id'];	
						...
						$result1= mysql_query("SELECT v.*, a.aer_nombre, d.des_nombre FROM  via v, aerolinea a, destino d WHERE v.fk_des_id=d.des_id AND v.fk_aer_id=a.aer_id AND v.fk_aer_id=$devuelve AND v.fk_des_id<>$devuelve1 AND v.fk_cru_id IS NULL AND v.fk_ter_id IS NULL") or die (mysql_error());
	...
	//		  fechai=2011-06-06&fecha2=2011-06-16&selected=4&selected2=k&id=94
